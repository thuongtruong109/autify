...
3. Copy the `env.json` file to the same directory as the exe file (if not already present)
...
2. **Check infomation**: The application will automatically load store information from the `env.json` file
...
- The `env.json` file must be in the same folder as the exe file
...
- Check if the `env.json` file exists and ensure that it has the correct format with the fields: email, password, storeId
